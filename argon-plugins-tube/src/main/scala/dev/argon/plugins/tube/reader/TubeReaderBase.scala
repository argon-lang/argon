...
abstract class TubeReaderBase extends TubeZipReaderUtil {
  ...
  protected def asDeclarationImpl[TTube <: ArTube](tube: TTube & HasImplementation[IsImplementation]): Option[TTube & HasImplementation[true]]
  ...
  private object ZipResourceFactory extends ResourceFactory[context.Env, context.Error] {
    override def directoryResource(name: String): DirectoryResource[context.Env, context.Error, BinaryResource] =
      ZipDirRes(if name.endsWith("/") then name else name + "/")

    override def binaryResource(name: String): BinaryResource[context.Env, context.Error] =
      new BinaryResource[context.Env, context.Error] with Resource.WithoutFileName {
        override def asBytes: ZStream[context.Env, context.Error, Byte] =
    ...
    final class ZipDirRes(baseDir: String) extends DirectoryResource[context.Env, context.Error, BinaryResource] with Resource.WithoutFileName {
      override def contents: ZStream[context.Env, context.Error, DirectoryEntry[context.Env, context.Error, BinaryResource]] =
        ZStream.unwrap(
          TSet.empty[String].commit.map { seenDirs =>
            zip.entries
              .filter(_.path.startsWith(baseDir))
              .mapZIO { entry =>
                val slashIndex = entry.path.indexOf("/", baseDir.length + 1)
                if entry.path.indexOf("/", baseDir.length + 1) >= 0 then
                  ZIO.some(DirectoryEntry.File(entry.path.substring(baseDir.length + 1).nn, entry.value))
                else
                  val dirName = baseDir.substring(baseDir.length + 1, slashIndex).nn
                  STM.ifSTM(seenDirs.contains(dirName))(
                    onTrue = STM.none,
                    onFalse = seenDirs.put(dirName)
                      .as(Some(DirectoryEntry.Subdirectory(dirName, ZipDirRes(baseDir.substring(0, slashIndex + 1).nn)))),
                  ).commit
                end if
              }
              .collectSome
  ...
  private def decodeToml(toml: t.Toml): Comp[Toml] =
    toml.value match {
      case t.Toml.Value.IntValue(i) => ZIO.succeed(Toml.Int(i))
      case t.Toml.Value.StringValue(s) => ZIO.succeed(Toml.String(s))
      case t.Toml.Value.FloatValue(d) => ZIO.succeed(Toml.Float(d))
      case t.Toml.Value.BoolValue(b) => ZIO.succeed(Toml.Boolean(b))
      case t.Toml.Value.OffsetDateTime(odt) => ZIO.succeed(Toml.OffsetDateTime(odt))
      case t.Toml.Value.LocalDateTime(dt) => ZIO.succeed(Toml.LocalDateTime(dt))
      case t.Toml.Value.LocalDate(d) => ZIO.succeed(Toml.LocalDate(d))
      case t.Toml.Value.LocalTime(t) => ZIO.succeed(Toml.LocalTime(t))
      case t.Toml.Value.Array(arr) =>
        ZIO.foreach(arr.elements)(decodeToml).map(Toml.Array.apply)

      case t.Toml.Value.Table(table) =>
        ZIO.foreach(table.elements) { kvp =>
          for
            value <- decodeToml(kvp.value)
          yield kvp.key -> value
        }.map { pairs => Toml.Table(pairs.toMap) }

      case _: t.Toml.Value.Empty.type =>
        ZIO.fail(InvalidTube("Unknown TOML value"))
    }
  ...
  def asTube: Comp[ArTube & HasImplementation[IsImplementation]] =
    for
      ...
      decodedOptions <- ZIO.foreach(decodedOptions)(context.optionsCodec.decode(ZipResourceFactory))
    ...
    yield new ArTubeC {
      ...
      override def asDeclaration: Option[this.type & HasImplementation[true]] = asDeclarationImpl(this)
  ...
  private def loadModule(currentTube: ArTube & HasImplementation[IsImplementation])(path: ModulePath)(using ReferenceLoader): Comp[ArModule & HasImplementation[IsImplementation]] =
    ...
    for
      ...
      moduleInfo <- readEntryMessage[t.ModuleDefinition]()
  ...
  private def loadClass(classOwnership: ArClassC.Ownership[context.type, IsImplementation], id: BigInt)(using refLoader: ReferenceLoader): Comp[ArClass & HasOwner[classOwnership.type] & HasImplementation[IsImplementation]] =
    for
      classDef <- readEntryMessage[t.ClassDefinition](id)
  ...
  private def loadTrait(traitOwnership: ArTraitC.Ownership[context.type, IsImplementation], id: BigInt)(using refLoader: ReferenceLoader): Comp[ArTrait & HasOwner[traitOwnership.type] & HasImplementation[IsImplementation]] =
    for
      traitDef <- readEntryMessage[t.TraitDefinition](id)
  ...
  private def loadFunction(funcOwnership: ArFuncC.Ownership[context.type, IsImplementation], id: BigInt)(using refLoader: ReferenceLoader): Comp[ArFunc & HasOwner[funcOwnership.type] & HasImplementation[IsImplementation]] =
    for
      funcDef <- readEntryMessage[t.FunctionDefinition](id)
  ...
  private def loadMethod(methodOwnership: ArMethodC.Ownership[context.type, IsImplementation], id: BigInt)(using refLoader: ReferenceLoader): Comp[ArMethod & HasOwner[methodOwnership.type] & HasImplementation[IsImplementation]] =
    for
      methodDef <- readEntryMessage[t.MethodDefinition](id)
  ...
  private def loadClassConstructor(ctorOwnership: ClassConstructorC.Ownership[context.type, IsImplementation], id: BigInt)(using refLoader: ReferenceLoader): Comp[ClassConstructor & HasOwner[ctorOwnership.type] & HasImplementation[IsImplementation]] =
    for
      ctorDef <- readEntryMessage[t.ClassConstructorDefinition](id)
