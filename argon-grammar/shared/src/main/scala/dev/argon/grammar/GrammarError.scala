...
object GrammarError {
  ...
  final case class UnexpectedToken[TToken, TTokenCategory](expectedCategory: TTokenCategory, token: WithSource[TToken]) extends GrammarError[TToken, TTokenCategory] {
    override def location: SourceLocation = token.location
  }

  final case class UnexpectedEndOfFile[TTokenCategory](expectedCategory: TTokenCategory, position: FilePosition) extends GrammarError[Nothing, TTokenCategory] {
