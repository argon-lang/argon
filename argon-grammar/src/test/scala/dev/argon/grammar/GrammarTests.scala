  ...
  override def spec: Spec[Environment & Scope, Any] =
    suite(suiteName)(
      ...
      suite("Cut grammar")(
        ...
        test("succeed after cut has been reached") {
          assert(parse((numberToken(1) ++! numberToken(2)) | (numberToken(1) ++! numberToken(3)))(1, 2))(isRight(anything))
        },
        test("not recover when failure occurs after cut has been reached") {
          assert(parse((numberToken(1) ++! numberToken(2)) | (numberToken(1) ++! numberToken(3)))(1, 3))(isLeft(anything))
        },
        test("still fail after alternative") {
          assert(parse(
            (
              (numberToken(1) ++! numberToken(2)) |
                (numberToken(1) ++! numberToken(3))
            ) ++ numberToken(1) |
              (numberToken(1) ++ numberToken(2) ++ numberToken(1))
          )(1, 2, 2))(isLeft(anything))
        },
      ),
    )
