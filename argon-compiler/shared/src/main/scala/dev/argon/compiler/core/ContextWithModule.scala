...
trait ContextWithModule extends Context {
  override def module[TContext >: ContextWithModule.this.type <: WithRes[ResIndicator] : zio.Tag]: ZManaged[ModuleLoad[ResIndicator, TContext] with ResourceReader[ResIndicator] with SourceParser, ErrorList, ArModule[ContextWithModule.this.type, DeclarationPayloadSpecifier]] =
