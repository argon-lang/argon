...
object ArgonModuleLoader extends ModuleLoader {
    ...
    def impl[TComp[+_] : Compilation](implicit compEv: LeibnizK[TComp, context.Comp]): TComp[ArModule[context.type, TPayloadSpec]] = {
      ...
      for {
        ...
        module <- new ModuleCreator[context.type, TComp, TPayloadSpec] {
          ...
          (
          ...
          ): TComp[Map[Int, ModuleObjectLoadResult[TRefResult, TDefResult]]] =
            valueLens(binModule).toVector.zipWithIndex.traverse { case (moduleObject, i) =>
              ...
              }) : TComp[ModuleObjectLoadResult[TRefResult, TDefResult]])
                .map { i -> _ }
          ...
          (
            elementMap: TComp[Map[Int, ModuleObjectLoadResult[TRef, TDef]]]
          )(
  ...
  type TraitLoadResult[TContext <: Context, PayloadSpec[_, _]] = ModuleObjectLoadResult[ArTrait[TContext, ReferencePayloadSpecifier], ArTrait[TContext, PayloadSpec]]
  type ClassLoadResult[TContext <: Context, PayloadSpec[_, _]] = ModuleObjectLoadResult[ArClass[TContext, ReferencePayloadSpecifier], ArClass[TContext, PayloadSpec]]
  type DataCtorLoadResult[TContext <: Context, PayloadSpec[_, _]] = ModuleObjectLoadResult[DataConstructor[TContext, ReferencePayloadSpecifier], DataConstructor[TContext, PayloadSpec]]
  type FunctionLoadResult[TContext <: Context, PayloadSpec[_, _]] = ModuleObjectLoadResult[ArFunc[TContext, ReferencePayloadSpecifier], ArFunc[TContext, PayloadSpec]]
  type MethodLoadResult[TContext <: Context, PayloadSpec[_, _]] = ModuleObjectLoadResult[ArMethod[TContext, ReferencePayloadSpecifier], ArMethod[TContext, PayloadSpec]]
