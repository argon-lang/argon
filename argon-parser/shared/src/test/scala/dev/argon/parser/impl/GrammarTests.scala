...
abstract class GrammarTestsCommon extends GrammarTestHelpers with NumberTokenHelpers {

  private lazy val rightRec: TGrammar[Int] =
    numberToken(0) | numberToken(1) ++ rightRec --> { case (a, b) => a + b }

  def grammarTestSuite(suiteName: String): ZSpec[Any, Nothing, String, Any] =
    suite(suiteName)(
      suite("Token grammar")(
        test("fail for EOF") {
      ...
      suite("Repeated grammar (+~)")(
        ...
        test("succeed for correct token") {
          assert(parse(numberToken(7)+~)(7), isRight(equalTo((Vector[TToken](), NonEmptyVector.of(7)))))
        },
        test("succeed for 2 correct tokens") {
          assert(parse(numberToken(7)+~)(7, 7), isRight(equalTo((Vector[TToken](), NonEmptyVector.of(7, 7)))))
        },
      ),
      suite("Union grammar")(
        test("fail for EOF") {
...
object GrammarTestsEntireSequence extends DefaultRunnableSpec(
  new GrammarTestsCommon with GrammarTestHelpersEntireSequence {}
    .grammarTestSuite("Grammar parsing with entire sequence")
)

object GrammarTestsSingleTokens extends DefaultRunnableSpec(
  new GrammarTestsCommon with GrammarTestHelpersSingleTokens {}
    .grammarTestSuite("Grammar parsing with single tokens")
)
