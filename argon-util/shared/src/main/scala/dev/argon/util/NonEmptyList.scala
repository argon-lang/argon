...
object NonEmptyList:
  ...
  def unapply[A](l: NonEmptyList[A]): Some[(A, List[A])] =
    Some((l.head, l.tail))
...
given TraverseNonEmpty[NonEmptyList] with Monad[NonEmptyList] with
  override def flatMap[A, B](fa: NonEmptyList[A])(f: A => NonEmptyList[B]): NonEmptyList[B] = fa.flatMap(f).asInstanceOf[NonEmptyList[B]]
  ...
  override def traverse[F[+_]: Applicative, A, B](ca: NonEmptyList[A])(f: A => F[B]): F[NonEmptyList[B]] =
    Applicative[F].map2(f(ca.head), summon[Traverse[List]].traverse(ca.tail)(f)) { (h2, t2) => NonEmptyList.cons(h2, t2) }
