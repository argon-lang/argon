...
case object NNil extends NList[Zero, Nothing] {
  override def asCons[P <: Nat](using zeroIsSucc: Zero Is Succ[P]): NCons[P, Nothing] =
    Is.nothing(zeroIsSucc.both(Zero))
...
final case class NCons[P <: Nat, +A](head: A, tail: NList[P, A]) extends NList[Succ[P], A] {
  ...
  override def zip[B](other: NList[Succ[P], B]): NList[Succ[P], (A, B)] =
    NCons((head, other.head), tail.zip(other.tail))
  
  override def map[B](f: A => B): NList[Succ[P], B] =
    NCons(f(head), tail.map(f))
...
given NListFactory[Zero] with
  override def fromSeqPrefix[A](s: Seq[A]): Option[(NList[Zero, A], Seq[A])] =
    Some(NNil, s)
end given
