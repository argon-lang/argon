...
import dev.argon.errors.{CompilerError, ErrorLog, WithLocation, WithSource}
...
object ArgonLexerTests extends ZIOSpecDefault {
    ...
    suite("Lexer")(
      ...
      suite("Integers")(
        ...
        test("Invalid Decimal Integer") {
          for
            tokens <- lex("123A")
          yield assertTrue(tokens == Seq(Token.Error("123A")))
        },
        ...
        test("Invalid Binary Integer") {
          for
            tokens <- lex("0b101010A")
          yield assertTrue(tokens == Seq(Token.Error("0b101010A")))
        },
        test("Octal Integer") {
          for
        ...
        test("Invalid Octal Integer") {
          for
            tokens <- lex("0o1234567A")
          yield assertTrue(tokens == Seq(Token.Error("0o1234567A")))
        },
        ...
        test("Invalid Hex Integer") {
          for
            tokens <- lex("0x123456789abcdefG")
          yield assertTrue(tokens == Seq(Token.Error("0x123456789abcdefG")))
        },
      ),
      suite("Identifiers")(
  ...
  object DiscardErrorLog extends ErrorLog {
    ...
    ZIO.scoped[ErrorLog](
      for
        ...
        tokens <- ZStream.repeatZIO(lexer.nextToken(LexerMode.Normal))
          ...
          .runCollect
      yield tokens
    ).provideEnvironment(ZEnvironment(DiscardErrorLog))
