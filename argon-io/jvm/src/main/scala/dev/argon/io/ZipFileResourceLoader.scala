...
object ZipFileResourceLoader extends ResourceLoader[ZipFileResource] {
  override def load(id: ResourceId): ZipFileResource =
    val channel = id.asSeekableByteChannel.getOrElse {
        ZManaged.fromZIO(
  ...
  private def loadChannel(id: ResourceId, channel: UManaged[SeekableByteChannel]): ZipFileResource = new ZipFileResource {
  ...
  private final class ZipFileImpl(zip: ACZipFile) extends ZipFile {
    override def entry(path: String): UIO[Option[ZipEntry]] = IO.succeed { Option(zip.getEntry(path)).map(new ZipEntryImpl(zip, _)) }
