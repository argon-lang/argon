...
pub fn error<G: GrammarTypes>() -> SymbolInfo<G> {
    SymbolInfo {
        symbol_type: SymbolInfoType::Error,
        with_location: false,
        discard: true,
    }
}
...
impl<G: GrammarTypes, X> GrammarBuilder<G, X> {
    ...
    pub fn error_token(&mut self) -> SymbolBuilder<X> {
        SymbolBuilder {
            sym: Symbol {
    ...
    while needs_factor {
        ...
        let type_for_sym = |sym: Symbol| {
            let mut t = match sym.symbol_type {
                ...
                SymbolType::Error => RuleType::Tuple(vec![]),
            };
