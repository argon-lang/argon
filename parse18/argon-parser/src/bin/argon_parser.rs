...
use parse18_ll_gen::grammar::{error, nonterm, rule, ruleset, term, Grammar, GrammarFactory, GrammarTypes, RuleInfo, RuleSetInfo};
...
pub enum Token {
    ...
    KwCase,
    #[strum(serialize = "Token.KW_LOOP.type")]
    KwLoop,
    #[strum(serialize = "Token.KW_WHILE.type")]
    KwWhile,
    #[strum(serialize = "Token.KW_BREAK.type")]
    KwBreak,
    #[strum(serialize = "Token.KW_NEXT.type")]
    KwNext,
    #[strum(serialize = "Token.KW_REDO.type")]
    KwRedo,
    ...
    fn create_rule_set(&self, r: Self::Rule) -> RuleSetInfo<Self> {
        match r {
            ...
            LoopExpr => ruleset(
                "Expr",
                [
                    rule([ term(KwLoop).discard(), nonterm(LoopLabel), nonterm(StatementList).with_location(), term(KwEnd).discard() ], "Expr.Loop"),
                ],
            ),
            ...
            WhileExpr => ruleset(
                "Expr",
                [
                    rule([ term(KwWhile).discard(), nonterm(LoopLabel), nonterm(StatementList).with_location(), term(KwDo).discard(), nonterm(StatementList).with_location(), term(KwEnd).discard() ], "Expr.While"),
                    rule([ term(KwWhile).discard(), nonterm(LoopLabel), nonterm(StatementList).with_location(), term(KwEnd).with_location() ], "((label: Option[WithSource[IdentifierExpr]], cond: WithSource[Seq[WithSource[Stmt]]], endKeyword: WithSource[?]) => Expr.While(label, cond, WithLocation(Seq(), endKeyword.location)))"),
                ],
            ),

            LoopLabel => ruleset(
                "Option[WithSource[IdentifierExpr]]",
                [
            ...
            BreakExpr => ruleset(
                "Expr",
                [
                    rule([ term(KwBreak).discard(), nonterm(LoopLabel) ], "Expr.Break"),
                ]
            ),
            ...
            NextExpr => ruleset(
                "Expr",
                [
                    rule([ term(KwNext).discard(), nonterm(LoopLabel) ], "Expr.Next"),
                ]
            ),
            ...
            RedoExpr => ruleset(
                "Expr",
                [
                    rule([ term(KwRedo).discard(), nonterm(LoopLabel) ], "Expr.Redo"),
                ]
            ),
            ...
            RaiseExpr => ruleset(
                ...
                [
                    ...
                    rule([ term(KwRaise).discard(), nonterm(AsExpr).with_location() ], "Expr.Raise"),
                    expr_error(),
                ],
            ),
            ClosureExpr => ruleset(
...
pub fn main() {
    ...
    let scala_settings = ScalaSettings {
        ...
        header_stmts: vec![
            ...
            "import dev.argon.errors.{WithSource, WithLocation}".to_owned(),
