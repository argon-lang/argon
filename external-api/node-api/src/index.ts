...
import { ServerFunctions, FileInfo, ServerFunctionCallHandler, MethodCallHandler, ExecutionResult } from "./api.gen";
import { WritableStream } from "memory-streams";
import * as vm from "vm";
import { Console } from "console";

const serverFunctions: ServerFunctions = {
	async executeJS(entrypoint: string, files: Array<FileInfo>): Promise<ExecutionResult> {
		try {
			...
			return { enumId: "Success", output: stdout.toString() };
		}
		catch(e) {
			return { enumId: "Failure", error: String(e) };
		}
	},
};
