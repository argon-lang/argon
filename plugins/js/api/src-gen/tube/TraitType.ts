...
export namespace V1 {
	export const codec: Codec<V1> = {
		async read(reader: FormatReader): Promise<V1> {
			return { id: await sym_int.codec.read(reader), arguments: await sym_list.codec<sym_argon_tube_Expression.V1>(sym_argon_tube_Expression.V1.codec).read(reader), };
		},
		async write(writer: FormatWriter, value: V1): Promise<void> {
			await sym_int.codec.write(writer, value.id);
			await sym_list.codec<sym_argon_tube_Expression.V1>(sym_argon_tube_Expression.V1.codec).write(writer, value.arguments);
		},
	};
}
