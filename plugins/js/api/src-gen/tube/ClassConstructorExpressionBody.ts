...
export interface V1 {
	...
	readonly preInit: sym_list.List<sym_argon_tube_PreInitClassConstructorStatement.V1>;
	readonly baseConstructorCall: sym_argon_tube_BaseClassConstructorCall.V1;
	readonly endExpr: sym_argon_tube_Expression.V1;
}
export namespace V1 {
	export const codec: Codec<V1> = {
		async read(reader: FormatReader): Promise<V1> {
			return { variables: await sym_list.codec<sym_argon_tube_LocalVariableDeclaration.V1>(sym_argon_tube_LocalVariableDeclaration.V1.codec).read(reader), preInit: await sym_list.codec<sym_argon_tube_PreInitClassConstructorStatement.V1>(sym_argon_tube_PreInitClassConstructorStatement.V1.codec).read(reader), baseConstructorCall: await sym_argon_tube_BaseClassConstructorCall.V1.codec.read(reader), endExpr: await sym_argon_tube_Expression.V1.codec.read(reader), };
		},
		async write(writer: FormatWriter, value: V1): Promise<void> {
			...
			await sym_list.codec<sym_argon_tube_PreInitClassConstructorStatement.V1>(sym_argon_tube_PreInitClassConstructorStatement.V1.codec).write(writer, value.preInit);
