...
export type Resource = DirectoryResource | BinaryResource;
...
export type OptionAddValueResult<Options> =
    { result: "success"; updated: Partial<Options>; } |
    { result: "invalid" } |
    { result: "duplicate" };

export abstract class OptionInfoBase {
    abstract readonly name: string;
    abstract readonly description: string;
}

export abstract class OptionInfoValue<Options> extends OptionInfoBase {
    abstract addOptionValue(prev: Partial<Options>, value: string): OptionAddValueResult<Options>;
}

export abstract class OptionInfoResource<Options> extends OptionInfoBase {
    abstract addOptionValue(prev: Partial<Options>, value: BinaryResource): OptionAddValueResult<Options>;
}

export type OptionInfo<Options> = OptionInfoValue<Options> | OptionInfoResource<Options>

export type OptionsBuildResult<Options> =
    { success: true; options: Options; } |
    { success: false; missing_value: string; };
...
export interface BuildOutputExecutor<Output> {
    execute(libraries: readonly { tubeName: readonly string[]; output: Output; }[], buildOutput: Output): Promise<{ exitCode: number; output: string; }>;
}
