...
export interface V1 {
	readonly owner: sym_argon_tube_ClassOwner.V1<sym_nat.Nat>;
...
export namespace V1 {
	export const codec: Codec<V1> = {
		async read(reader: FormatReader): Promise<V1> {
			return { owner: await sym_argon_tube_ClassOwner.V1.codec<sym_nat.Nat>(sym_nat.codec).read(reader), name: await sym_option.codec<sym_argon_tube_Identifier.V1>(sym_argon_tube_Identifier.V1.codec).read(reader), signature: await sym_argon_tube_ErasedSignatureParameterOnly.V1.codec.read(reader), };
		},
		async write(writer: FormatWriter, value: V1): Promise<void> {
			await sym_argon_tube_ClassOwner.V1.codec<sym_nat.Nat>(sym_nat.codec).write(writer, value.owner);
