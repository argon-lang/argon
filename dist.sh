...
EMSDK_DIR="$PWD/.emsdk/$EMSCRIPTEN_VERSION"
...
PATH="$EMSDK_DIR/upstream/emscripten:$EMSDK_DIR/upstream/bin:$PATH" \
	CXXFLAGS_wasm32_unknown_emscripten=-fwasm-exceptions \
	cargo build --bin argonc --release --target wasm32-unknown-emscripten
...
mkdir dist/arch/wasm32-unknown-emscripten/
mkdir dist/backend/
...
cp -p target/wasm32-unknown-emscripten/release/argonc.js dist/arch/wasm32-unknown-emscripten/
cp -p target/wasm32-unknown-emscripten/release/argonc.wasm dist/arch/wasm32-unknown-emscripten/
