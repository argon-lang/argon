...
abstract class PrologContext[R <: Random, E] {
  ...
  protected def intrinsicPredicate(predicate: TPredicateFunction, args: Seq[Expr], substitutions: Model, fuel: Int): ZStream[R, Error, PrologResult.Yes]
  ...
  protected def checkRelation(a: syntax.Expr, b: syntax.Expr, relation: TRelation, substitutions: Model, fuel: Int): ZStream[R, Error, PrologResult.Yes]
  ...
  protected def valueRelationProof(relation: TRelation, a: syntax.Value, b: syntax.Value, argProofs: Seq[Proof[ProofAtom]]): ZIO[R, E, Proof[ProofAtom]]
  ...
  final def check(goal: Predicate, fuel: Int): ZIO[R, E, PrologResult] =
    check(goal, Map.empty, fuel)
  ...
  protected def solve(goal: Predicate, substitutions: Model, fuel: Int): ZStream[R, Error, PrologResult.Yes] =
    inferAll(goal, substitutions, fuel) ++ (goal match {
      ...
      case Or(a, b) =>
        val commuteOr = inferAll(Or(b, a), substitutions, fuel)
          .mapError {
            _.map {
              case PrologResult.No(proof, model) =>
                PrologResult.No(Proof.DeMorganAndPullNotOut(Proof.ConjunctCommute(Proof.DeMorganOrPushNotIn(proof))), model)
            }
          }
          .map { case PrologResult.Yes(proof, substitutions) => PrologResult.Yes(Proof.DisjunctCommute(proof), substitutions) }

        commuteOr
          ++ emptyIfNo(solve(a, substitutions, fuel)).map { case PrologResult.Yes(proof, substitutions) => PrologResult.Yes(Proof.DisjunctIntroLeft(proof), substitutions) }
          ++ emptyIfNo(solve(b, substitutions, fuel)).map { case PrologResult.Yes(proof, substitutions) => PrologResult.Yes(Proof.DisjunctIntroRight(proof), substitutions) }
      ...
      case Implies(And(a, b), PropFalse) =>
        solve(Or(Implies(a, PropFalse), Implies(b, PropFalse)), substitutions, fuel)
          ...
          .map { case PrologResult.Yes(proof, substitutions) => PrologResult.Yes(Proof.DeMorganOrPullNotOut(proof), substitutions) }

      case Implies(Or(a, b), PropFalse) => solve(And(Implies(a, PropFalse), Implies(b, PropFalse)), substitutions, fuel).map { case PrologResult.Yes(proof, substitutions) => PrologResult.Yes(Proof.DeMorganAndPullNotOut(proof), substitutions) }

      case Implies(Implies(a, PropFalse), PropFalse) => solve(a, substitutions, fuel).map { case PrologResult.Yes(proof, substitutions) => PrologResult.Yes(Proof.DoubleNegIntro(proof), substitutions) }
        ...
        ZStream.unwrap(
          ...
          emptyIfNo(
            solve(Implies(predicate, PropFalse), substitutions, fuel - 1)
              .map { case PrologResult.Yes(notProof, substitutions) => PrologResult.Yes(Proof.Contradiction(proof, notProof), substitutions) }
          )
        }
  ...
  private def infer(goal: Predicate, substitutions: Model, kb: List[(Proof[ProofAtom], Predicate)], fuel: Int): ZStream[R, Error, PrologResult.Yes] =
    kb match {
  ...
  private def inferOne(goal: Predicate, assertion: Predicate, substitutions: Model, proof: Proof[ProofAtom], fuel: Int): ZStream[R, Error, PrologResult.Yes] = {
    val unifyAssertion =
      unify(goal, assertion, substitutions, fuel)
        .map { model => PrologResult.Yes(proof, model) }

    val assertionSpecific = assertion match {
      case Implies(premise, conclusion) =>
        val implicationCheck = premise match {
  ...
  private def unify(goal: Predicate, rule: Predicate, substitutions: Model, fuel: Int): ZStream[R, Error, Model] = {
    (goal, rule) match {
      case (pf1 @ PredicateFunction(f1, args1), pf2 @ PredicateFunction(f2, args2)) if f1 == f2 && args1.size == args2.size =>
        ZStream.unwrap(
  ...
  private def unifyExpr(goal: Expr, rule: Expr, relation: TRelation, substitutions: Model, fuel: Int, useCustomRelations: Boolean): ZStream[R, Error, PrologResult.Yes] =
    if fuel <= 0 then
      Stream.empty
    else
      val customRelations = checkRelation(goal, rule, relation, substitutions, fuel - 1)

      def unifyVariable(variable: TVariable, other: syntax.Expr, relation: TRelation): ZStream[R, Error, PrologResult.Yes] =
        ZStream.unwrap(
                      ...
                      args1.zip(args2).zip(relations).toList.foldLeftM((substitutions, Seq.empty[Proof[ProofAtom]])) {
                        case ((substitutions, argProofs), ((arg1, arg2), argRelation)) =>
                          ...
                          unifyExpr(arg1, arg2, mergedRel, substitutions, fuel, useCustomRelations = true)
                            .map { case PrologResult.Yes(argProof, substitutions) => (substitutions, argProofs :+ argProof) }
                      }
  ...
  private def addSubstitution(substitutions: Model, variable: TVariable, constraints: TConstraints): ZStream[R, Error, Model] =
    Stream.fromIterable(
