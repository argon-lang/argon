  ...
  override def spec: ZSpec[Environment, Failure] =
    suite("Contradiction Proofs")(
      testM("A or B, not A, not B") {
        val prologContext = new ProofPrologContext[VariableProvider & zio.Random, Nothing] with ProofAssertions[VariableProvider & zio.Random, Nothing] {
          override val syntax: prologSyntax.type = prologSyntax

          override protected def assertions: URIO[VariableProvider, List[(Proof[String], syntax.Predicate)]] = IO.succeed(List(
        ...
        assertM(prologContext.check(PropFalse, fuel))(
          hasProof(Proof.Contradiction(Proof.Atomic("disjunct"), Proof.DeMorganAndPullNotOut(Proof.ConjunctIntro(Proof.Atomic("nota"), Proof.Atomic("notb")))))
        )
      },
