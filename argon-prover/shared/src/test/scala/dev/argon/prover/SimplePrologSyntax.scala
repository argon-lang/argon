...
class SimplePrologSyntax[PredFunc, Constructor](using
  ...
  def expr(ctor: TConstructor, args: Expr*): Expr =
    Value(ctor, args)

  def pred(predicateFunction: PredFunc, args: Expr*): Predicate =
    PredicateFunction(predicateFunction, args)
