...
abstract class SimplePrologContext[PredFunc, Constructor](using CanEqual[PredFunc, PredFunc], CanEqual[Constructor, Constructor])
  ...
  override protected def unifyPredicateExpression(f1: PredicateApply, f2: PredicateApply, model: Ref[Model], fuel: Int): ZIO[VariableProvider, Nothing, Boolean] =
    ZIO.succeed(f1.name == f2.name && f1.args.length == f2.args.length) &&
      f1.args.zip(f2.args).forallM { case (a, b) => unifyExpr(a, b, model, fuel) }

  private def unifyExpr(a: Expr, b: Expr, model: Ref[Model], fuel: Int): UIO[Boolean] =
    if fuel <= 0 then
      ZIO.succeed(false)
    else
      (a, b) match {
  ...
  private def unifyVariable(a: String, b: Expr, model: Ref[Model], fuel: Int): UIO[Boolean] =
    model.get.flatMap { m =>
      m.get(a) match {
        case Some(a2) => unifyExpr(a2, b, model, fuel - 1)
        case None =>
