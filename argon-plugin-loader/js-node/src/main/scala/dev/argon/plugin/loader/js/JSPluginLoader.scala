...
class JSPluginLoader[R, E >: InvalidTube] extends PluginLoader[R, E] {
  ...
  override def decodeConfig(path: PathLike, elem: xml.Element): Either[String, JSPluginConfig] =
    for
      module <- elem.child(xml.Name("ModulePath")).map { modPath => PathLike.join(path, modPath.textContent) }.toRight("ModulePath not specified")
      exportName <- elem.child(xml.Name("ExportName")).map { _.textContent }.toRight("ExportName not specified")
