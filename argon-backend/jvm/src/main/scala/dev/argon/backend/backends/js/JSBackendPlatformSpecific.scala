...
trait JSBackendPlatformSpecific[E >: BackendException | IOException] {
  ...
  val codeGenerator: CodeGenerator.LibraryCodeGenerator[E, JSOutput] { type Options = JSOptions } =
        ...
        class JSCodegenImpl(jsContext: GraalContext, jExecutor: JExecutor, executor: Executor)(using runtime: Runtime[Any]) {
                      ...
                      Unsafe.unsafely {
                        runtime.unsafe.fork(io)
                          .unsafe.addObserver {
                            ...
                            case Exit.Failure(cause) => jExecutor.execute(() => reject.execute(errorContext.errorWrapper.wrap(cause)))
                          }
                      }
                      ...
                      arguments match {
                        case Seq(result) =>
                          if result.isException() then
                            ...
                            register(ZIO.fail(errorContext.errorWrapper.wrap(Cause.fail(JavaScriptException(result.toString())))))
          ...
          def asyncIterableToStream(asyncIterableThunk: => Value): Stream[E, Value] =
            ZStream.unwrapScoped(
              for
                asyncIterable <- attemptJSContextTask {
                  asyncIterableThunk
                }.orDie
              ...
              yield (
                ZStream.repeatZIOChunkOption(
                  promiseToTask {
                    asyncIterable.invokeMember("next")
                  }
                    .flatMap { res =>
                      attemptJSContextTask {
        ...
        def runCodegen: Stream[E, ModuleCodegenResult] =
          ZStream.unwrapScoped(
            for
              ...
              jExecutor <- ZIO.fromAutoCloseable(ZIO.succeed {
