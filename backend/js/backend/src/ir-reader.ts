...
} from "@argon-lang/js-backend-api/vm.js";
import {getModuleId, tubeNameEquals} from "./util.js";
import {JsPlatformTubeMetadata, type InputFile} from "@argon-lang/js-backend-api";
...
class IrReader {
    ...
    build(): ProgramModel {
        ...
        let decodedMetadata: DecodedMetadata = {
            platformMetadata: decodePlatformMetadata(this.metadata.platformMetadata),
            referencePlatformMetadata: this.metadata.referencedTubes.map(rt => ({
                tubeName: rt.name,
                platformMetadata: decodePlatformMetadata(rt.metadata),
            })),
        };
...
function decodePlatformMetadata(expr: ESExpr | undefined): JsPlatformTubeMetadata {
    if(expr === undefined) {
        return {
    ...
    const pm = JsPlatformTubeMetadata.codec.decode(expr);
    if(!pm.success) {
        throw new Error(`Failed to decode platform metadata: ${pm.message}`);
    }
    return pm.value;
}
...
export async function readIRFile(file: InputFile): Promise<ProgramModel> {
    return readIR(decodeEntries(readExprStream(readDataStream(file))));
}

async function* readDataStream(file: InputFile): AsyncGenerator<Uint8Array> {
...
async function* decodeEntries(entries: AsyncIterable<ESExpr>): AsyncIterable<TubeFileEntry> {
    for await(const entry of entries) {
        const result = TubeFileEntry.codec.decode(entry);
        if(!result.success) {
            throw new Error(`Failed to decode TubeFileEntry: ${result.message}`);
        }
...
class ProgramModelImpl implements ProgramModel {
    constructor(options: ProgramModelOptions) {
        ...
        this.decodedMetadata = options.decodedMetadata;
    ...
    getTubeInfo(id: bigint): TubeInfo {
        if(id === 0n) {
            return {
                ...
                platformMetadata: this.decodedMetadata.platformMetadata,
            };
        }
        else {
            const tubeRef = this.metadata.referencedTubes[Number(id - 1n)];
            if(tubeRef === undefined) {
                throw new Error("Invalid tube id");
            }

            const platformMetadata: JsPlatformTubeMetadata = this.decodedMetadata.referencePlatformMetadata
                .find((meta) => tubeNameEquals(tubeRef.name, meta.tubeName))
                ?.platformMetadata
                ?? {
                    packageName: undefined,
                };
