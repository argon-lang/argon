...
import type {TubeInfo} from "./program-model.js";
...
export function tubePackageName(tubeInfo: TubeInfo): string {
    if(tubeInfo.platformMetadata.packageName !== undefined) {
        return tubeInfo.platformMetadata.packageName;
    }

    const name = tubeInfo.tubeName;
