...
public final class ExternGeneratorSupport {
	...
	public static RuntimeInvisibleAnnotationsAttribute externFunction(String externName) {
		return RuntimeInvisibleAnnotationsAttribute.of(Annotation.of(
			CD_EXTERN_FUNCTION,
			AnnotationElement.of("value", AnnotationValue.of(externName))
		));
	}

	public static byte[] staticExternClass(
		ClassDesc className,
		int classFlags,
		String externName,
		String methodName,
		...
		return CLASS_FILE.build(className, classBuilder -> classBuilder
			.withFlags(classFlags)
			.withMethod(methodName, descriptor, ClassFile.ACC_PUBLIC | ClassFile.ACC_STATIC, methodBuilder -> methodBuilder
				.with(externFunction(externName))
				.withCode(body)
			));
	}
}
