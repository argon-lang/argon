...
public final class Main {
    ...
    static int run(String[] args, PrintStream diagnostics) {
        ...
        for (int i = 0; i < args.length; i += 2) {
            if (i + 1 >= args.length) return usage(diagnostics, "Missing value for " + args[i]);
            switch (args[i]) {
                case "--sources" -> sources = Path.of(args[i + 1]);
                case "--output" -> output = Path.of(args[i + 1]);
                default -> { return usage(diagnostics, "Unknown option: " + args[i]); }
            }
        }
        if (sources == null || output == null) return usage(diagnostics, "Both options are required");
        ...
        try {
            ...
            if (compiledGenerators == null) return 1;
            Map<String, byte[]> classes = generate(compiledGenerators);
                if (classes.isEmpty()) throw new IllegalArgumentException("No ExternGenerator implementations found");

                Path outputParent = output.toAbsolutePath().getParent();
                if (outputParent == null) throw new IllegalArgumentException("Output has no parent: " + output);
    ...
    private static Map<String, byte[]> compile(List<Path> sources, PrintStream diagnostics) throws IOException {
        ...
        if (compiler == null) throw new IllegalStateException("No system Java compiler is available");
        Path modulePath;
        try {
        ...
        try (StandardJavaFileManager standardManager = compiler.getStandardFileManager(null, null, null);
            ...
            boolean success = compiler.getTask(
                    new java.io.PrintWriter(diagnostics, true), manager, null, options, null, compilationUnits).call();
    ...
    private static Map<String, byte[]> generate(Map<String, byte[]> compiledClasses) throws Exception {
        ...
        if (moduleReferences.size() != 1) {
            throw new IllegalArgumentException("Expected exactly one extern generator module, found " + moduleReferences.size());
        }
        ...
        var configuration = ModuleLayer.boot().configuration().resolveAndBind(
                finder, ModuleFinder.of(), List.of(moduleName));
        ...
        ModuleLayer layer = ModuleLayer.defineModules(
                configuration, List.of(ModuleLayer.boot()), _moduleName -> loader).layer();
        ...
        if (Main.class.getModule().isNamed()) {
            ...
            generators = moduleReferences.iterator().next().descriptor().provides().stream()
                    ...
                    .sorted()
                    .map(providerName -> instantiate(layer.findLoader(moduleName), providerName))
                    .toList();
        }
        ...
        for (Object generator : generators) {
                ...
                var generated = (java.util.Collection<?>)java.util.Objects.requireNonNull(
                        value, "Generator returned null: " + generatorName);
                for (Object item : generated) {
                    byte[] bytes = (byte[])java.util.Objects.requireNonNull(
                            item, "Generator returned a null classfile: " + generatorName);
    ...
    private static final class MemoryModuleFinder implements ModuleFinder {
        ...
        private MemoryModuleFinder(Map<String, byte[]> classes) {
            ...
            if (moduleInfo == null) throw new IllegalArgumentException("Extern generators must declare a module");
    ...
    private static final class MemoryModuleReader implements ModuleReader {
        private final Map<String, byte[]> resources;
        private MemoryModuleReader(Map<String, byte[]> resources) { this.resources = resources; }
        @Override public Optional<URI> find(String name) {
