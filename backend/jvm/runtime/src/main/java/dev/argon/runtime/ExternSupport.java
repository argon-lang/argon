...
/** Runtime implementations used by generated standard-library extern entry points. */
public final class ExternSupport {
	...
	private static Trampoline<Object> openRead(String path) {
		try {
			...
			Object value = Proxy.newProxyInstance(
				...
				(_proxy, method, args) -> {
					...
					try {
						int result = stream.read((byte[])nonNullArgs[0], ((BigInteger)nonNullArgs[1]).intValueExact(), ((BigInteger)nonNullArgs[2]).intValueExact());
		...
		catch(ClassNotFoundException ex) { throw new IllegalStateException("Argon.IO InputStream interface is missing", ex); }
	}
	...
	private static Trampoline<Object> openWrite(String path) {
		try {
			...
			Object value = Proxy.newProxyInstance(
				...
				(_proxy, method, args) -> {
					...
					var nonNullArgs = java.util.Objects.requireNonNull(args);
					try {
						stream.write((byte[])nonNullArgs[0], ((BigInteger)nonNullArgs[1]).intValueExact(), ((BigInteger)nonNullArgs[2]).intValueExact());
						return new Trampoline.Result<>(new Tuple0());
					}
					catch(IOException ex) { throw new UncheckedIOException(ex); }
				}
		...
		catch(ClassNotFoundException ex) { throw new IllegalStateException("Argon.IO OutputStream interface is missing", ex); }
	}
	...
	private static Object resource(Object stream, Class<?> streamType, CloseOperation close) {
		...
		catch(ClassNotFoundException ex) { throw new IllegalStateException("Argon.IO Resource interface is missing", ex); }
	}

	@FunctionalInterface
	private interface CloseOperation { void close() throws IOException; }
}
