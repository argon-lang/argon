...
export async function loadExterns(files: readonly InputFile[]): Promise<Map<string, ESExpr>> {
    ...
    for(const file of files) {
        ...
        for(const extern of moduleExternLoader.load(jsFile)) {
            if(externs.has(extern.name)) {
...
async function readJSFile(file: InputFile): Promise<estree.Program> {
    ...
    try {
        ...
        for(;;) {
            ...
            sourceCode += decoder.decode(buffer.subarray(0, bytesRead), { stream: true });
        }

        sourceCode += decoder.decode();
