...
object OptionsHandlerImpl {
  ...
  sealed trait OptionIDListPart[O <: OptionID] {
    val id: O
    def combineRepr[A[_], B[_], C[_], F[+_]: Applicative](lista: A[O], listb: B[O])(f: CombineFunction[O, A, B, C, F]): F[C[O]]
  }
  ...
  implicit inline def optionIDList[O <: OptionID](implicit mirror: Mirror.SumOf[O]): OptionIDList[O] = {
    ...
    new OptionIDList[O] {
      ...
      override def combineRepr[A[_], B[_], C[_], F[+_]: Applicative](lista: List[Any], listb: List[Any])(f: CombineFunction[O, A, B, C, F]): F[List[Any]] = {
      ...
      override def reprToOptions[A[_]](list: List[Any]): Options[A, O] = {
        Options.fromFunction(new Options.OptionValueFunction[A, O] {
          override def apply[E](id: O with TypedOptionID[E]): A[E] =
            list(mirror.ordinal(id)).asInstanceOf[A[E]]
        })
      }
    }
  }
