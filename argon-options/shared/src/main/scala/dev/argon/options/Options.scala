...
object Options {
  ...
  def fromFunction[A[_], O <: OptionID](f: OptionValueFunction[A, O]): Options[A, O] =
    new FunctionBackedOptionOptions(f)

  private class FunctionBackedOptionOptions[A[_], O <: OptionID] private (getDefault: OptionValueFunction[A, O], map: Map[O, Any]) extends Options[A, O] {
  ...
  private[options] final class MapBackedOptionOptions[A[_], O <: OptionID](map: Map[O, Any]) extends Options[[X] =>> Option[A[X]], O] {
