...
final case class JSBackend(private val moduleExtractor: JSModuleExtractor) extends Backend {
  ...
  override def compile[I <: ResourceIndicator: Tag](input: CompilerInput[I, JSBackendOptions[Id, I]]): ZManaged[ResourceReader[I] with SourceParser, CompilationError, TCompilationOutput] = for {
    externCache <- ZManaged.fromEffect(ValueCache.make[CompilationError, Map[String, ResolvedExtern]])
    ...
    context: JSContext with Context.WithRes[I] = new JSContext {
      ...
      override protected val externFunctionsCache: ValueCache[CompilationError, Map[String, ResolvedExtern]] = externCache
  ...
  private def createOutput(jsModule: JSModule): CompilationOutputText[BackendOutputOptionsId] = new CompilationOutputText[BackendOutputOptionsId] {
    ...
    override val textStream: Stream[CompilationError, String] =
