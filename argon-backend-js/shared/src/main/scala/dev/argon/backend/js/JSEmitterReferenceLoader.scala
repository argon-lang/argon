      ...
      if(emitEnv.module.id.name === LookupNames.argonCoreLib)
        ...
        IO.fromEither(
          emitEnv.modulePairs
            ...
            .toRight { DiagnosticError.ReferencedModuleNotFound(dev.argon.module.ModuleReference(LookupNames.argonCoreLib), DiagnosticSource.EmitPhase()) }
        )
    ...
    ZIO.accessM[EmitEnv] { emitEnv =>
      if(emitEnv.module.id === moduleId)
        ...
        IO.fromOption(
          ...
          .mapError[CompilationError] { _ =>
            DiagnosticError.ReferencedModuleNotFound(dev.argon.module.ModuleReference(LookupNames.argonCoreLib), DiagnosticSource.EmitPhase())
          }
