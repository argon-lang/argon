...
object SourceTrait {
  ...
  : ctx.Comp[ArTraitC with HasContext[ctx.type] with HasOwner[TOwner]] =
    ...
    for {
      ...
      sigCell <- MemoCell.make[CompEnv, CompError, Signature[ctx.ExprContext.WrapExpr, (ctx.ExprContext.WrapExpr, Seq[ctx.ExprContext.ArExpr[ctx.ExprContext.ExprConstructor.TraitType]])]]
      methodsCell <- MemoCell.make[CompEnv, CompError, Map[Option[IdentifierExpr], Seq[ArMethodC with HasContext[ctx.type] with HasOwner[OwnedByTraitC[ctx.type, traitOwner.type]]]]]
      staticMethodsCell <- MemoCell.make[CompEnv, CompError, Map[Option[IdentifierExpr], Seq[ArMethodC with HasContext[ctx.type] with HasOwner[OwnedByTraitStaticC[ctx.type, traitOwner.type]]]]]
      ...
      override def signature: Comp[Signature[WrapExpr, TraitResult]] =
        sigCell.get(
          outerEnv.flatMap { env =>
            SignatureUtil.create(context)(exprConverter)(env)(stmt.parameters)(SignatureUtil.createTraitResult(context)(exprConverter)(stmt))
              .flatMap { (sig, env) =>
      ...
      override def staticMethods: Comp[Map[Option[IdentifierExpr], Seq[ArMethod with HasOwner[OwnedByTraitStatic[owner.type]]]]] =
